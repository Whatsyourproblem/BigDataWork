...
import '@/api/index'
import {getCityInfoByProvince, getProvinceInfo} from "@/api";
...
export default {
  ...
  setup() {
    ...
    const getDown = async (name) => {
      ...
      if (name === '上海市' || name === '北京市' || name === '重庆市' || name === '天津市' || name === '香港特别行政区' || name === '澳门特别行政区'){
          // 不进行任何操作
          console.log(name)
      } else {
          //获取地图数据之后，获取地图数据
